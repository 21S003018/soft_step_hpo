...
from model.nas_model.layers.singlepath import SinglePathChannelConv2d, SinglePathKernelConv2d
...
class LinearBlock(nn.Module):  # normal block or reduction block
    def __init__(self, in_planes, out_planes, kernel_size=3, stride=1, expansion=4):
        ...
        self.conv1 = SinglePathChannelConv2d(
...
class SinglePath(nn.Module):
    def __init__(self, input_channel, ndim, num_classes, path=None) -> None:
        ...
        self.conv_in, self.bn_in = SinglePathChannelConv2d(
        ...
        self.conv_out, self.bn_out = SinglePathChannelConv2d(
...
class SkipBottleneck(nn.Module):  # skip block
    def __init__(self, in_planes, hidden_planes, kernel_size=3, stride=1, expansion=4):
        ...
        self.conv2 = SinglePathKernelConv2d(hidden_planes, hidden_planes, kernel_size=kernel_size, stride=stride, padding=int(
...
class BottleneckSinglePath(nn.Module):
    def __init__(self, input_channel, ndim, num_classes, path=None) -> None:
        super(BottleneckSinglePath, self).__init__()
        with open(path, 'r') as f:
            config = json.load(f)
        self.block_type = config["type"]
        # pre conv
...
class ShallowSinglePath(nn.Module):
    def __init__(self, input_channel, ndim, num_classes, path=None) -> None:
        super(ShallowSinglePath, self).__init__()
        with open(path, 'r') as f:
            config = json.load(f)
        self.block_type = config["type"]
        # pre conv
...
if __name__ == '__main__':
    ...
    # model = SinglePath(input_channel, inputdim, nclass)
