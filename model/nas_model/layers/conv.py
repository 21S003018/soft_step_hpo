...
class SoftKernelConv2d(SoftConv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, bias=False, groups=1):
        ...
        self.kernel_size_vector = torch.ones((1, self.kernel_size))
        self.kernel_size_mask = Parameter(
            torch.ones((self.kernel_size, self.kernel_size)), requires_grad=False)
    ...
    def format_kernel_size(self):
        ...
        self.kernel_size_vector[0, 1:] = torch.stack(
            (fack_kernel_size_vector, fack_kernel_size_vector), dim=1).squeeze(0).T.flatten().unsqueeze(0)
        # construct matrix mask
        for i in range(self.kernel_size):
            self.kernel_size_mask[:self.kernel_size-i, :self.kernel_size -
                                  i] = self.kernel_size_vector[0][self.kernel_size-i-1]
        # calculate optimal mask
        self.opt_kernel_size = round(float(torch.sum(self.kernel_size_vector)))
        print(self.kernel_size_vector.requires_grad)
        self.opt_kernel_size_vector = self.kernel_size_vector.detach().numpy()
        return

    def forward(self, x):
        self.format_kernel_size()
        x = F.conv2d(x, weight=torch.mul(self.weight, self.kernel_size_mask), stride=self.stride,
                     padding=self.padding, bias=self.bias, groups=self.groups)
        return x
