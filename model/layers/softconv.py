...
class SoftConv2d(nn.Module):
    ...
    def reset_parameters(self):
        ...
        init.uniform_(self.channel_alpha, int(self.out_channels*0.5), int(self.out_channels*0.75))
        init.uniform_(self.kernel_alpha, int(self.kernel_size/2)*0.5, int(self.kernel_size/2)*0.75)
        return
    ...
    def forward(self, x):
        # tune kernel size
        indicators = self.sample_indicator(self.kernel_alpha, self.expansion, int(self.kernel_size/2))
        mask = torch.ones((self.kernel_size,self.kernel_size))
        for index, _ in enumerate(indicators):
            mask[:self.kernel_size-index*2,:self.kernel_size-index*2] = indicators[int(self.kernel_size/2)-index-1]
        masked_weight = torch.mul(self.weight, mask)
        # tune channel amount
        indicators = self.sample_indicator(self.channel_alpha, self.expansion, self.out_channels)
        masked_weight = torch.mul(masked_weight, indicators.reshape((indicators.shape[0],1,1,1)))
        # conv
...
class SoftChannelConv2d(nn.Module):
    ...
    def forward(self, x):
        indicators = self.sample_indicator(self.channel_alpha, self.expansion, self.out_channels)
        masked_weight = torch.mul(self.weight, indicators.reshape((indicators.shape[0],1,1,1)))
        x = F.conv2d(x, weight=masked_weight, stride=self.stride,padding=self.padding, groups=self.groups)
        return x
